...
tablePresenter.list = function () {
    ...
    request.post(rest.usu.usuario.findall_uri, dataInput).then(function (data) {
...
modalPresenter.submit = function () {
    ...
    self._view.apply(function (data1) {
        ...
        var id = self._view.ui.find("input[name='usuario[id]']").val();
        var route = (id > 0) ? rest.usu.usuario.update1_uri : rest.usu.usuario.create_uri;
...
modalPresenter.delete = function () {
    ...
    if (id > 0) {
        ...
        confirmMsg(NOTIFIERS.MESSAGES.DELETECONFIRM, function (dataConfirm) {
            if (dataConfirm) {
                request.post(rest.usu.usuario.delete_uri, {usuario: {id: parseInt(id)}}).then(function (data) {
...
modalPresenter.openEdit = function () {
    ...
    if (id > 0) {
        ...
        request.post(rest.usu.usuario.findByIdedit_uri, {usuario: {id: parseInt(id)}}).then(function (data) {
            if (data.result == 0)
            {
                self._view.fillDataModal1(data.usuario);
...
filterPresenter.submit = function () {
    ...
    {
        request.post(rest.usu.usuario.findall_uri, dataSerialize).then(function (data) {
